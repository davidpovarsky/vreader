...
| `.readerBilingualDidChange`    | `["fingerprintKey": String, "isEnabled": Bool, "targetLanguage": String, "granularity": String]` | `BilingualReadingViewModel` → format renderers + parent reader chrome (Feature #56 WI-7b/WI-10/WI-11 — bilingual toggled on/off, language changed, or a unit's translation became available; renderers re-inject or clear the interlinear translation, the parent `ReaderContainerView` mirrors `isEnabled` + `targetLanguage` into `bilingualActive` / `bilingualLanguage` so the `BilingualPill` and More-menu row paint without crossing the host boundary) |
| `.readerSessionTimeDidChange`  | `["fingerprintKey": String, "display": String]` | `ReaderLifecycleHelper.updateTimeDisplays` → `ReaderContainerView` (Feature #101 WI-2b — the ~1-post/min live session display, mirrored into book-keyed `currentSessionDisplay` so Book details' "This session" row has the latest value at sheet-open; `""` while the session formatter returns nil) |
| `.readerMoreTranslationSettings` | `["fingerprintKey": String, "bookTitle": String]` | More-menu settings row + bilingual pill → per-format bilingual hosts (Feature #99 — keyed re-entry; the host presents the edit-framed `BilingualSetupSheet`. Every More-menu row post now carries `fingerprintKey`; only this one REQUIRES filtering) |
| `.readerBilingualRetranslateStarted` | `["fingerprintKey": String, "language": String, "previousLanguage": String]` | Host edit-confirm → `ReaderContainerView` (Feature #99 — a NEW target language was applied; the container floats the transient `BilingualRetranslateBanner` under the top chrome, ~4s) |
| `.readerBilingualPrefetchDidChange` | `["fingerprintKey": String, "inFlightUnits": Set<TranslationUnitID>]` | `BilingualReadingViewModel.setInFlight(_:)` → format renderers (Feature #77 WI-1 — the single funnel for EVERY `inFlightUnits` mutation posts the FULL current in-flight set so a renderer can authoritatively draw or clear the inline loading shimmer. The full-set (not delta) payload is deliberate: a cancel/retry that removes a unit OUTSIDE the start/finish path would otherwise leak a stale shimmer) |
...
| `.readerAskAIRequested`        | `TextSelectionInfo` (`userInfo` may carry `selectionRequestToken`) | `SelectionPopoverActionRouter` ("Ask AI") → ReaderContainerView (Feature #78 — parks the selection in `pendingAskAIText`, targets `aiInitialTab = .chat`, then opens the AI panel (available) or routes to the #82 readiness sheet (unconfigured — seed survives the handoff); the `onChange(showAIPanel)` drain seeds the chat input AFTER `ensureAIReady`. Observed via `ReaderAskAIReadObserver`) |
| `.readerReadAloudRequested`    | `TextSelectionInfo` | `SelectionPopoverActionRouter` ("Read") → ReaderContainerView (Feature #78 — `ttsService.startSpeaking(text:fromOffset:)`, preempts any active book-reading session, no resume) |
...
- **Feature #177 core contract lane** — the standalone XcodeGen spec
  `Feature177Core.project.yml` defines a `Feature177CoreTests` logic-test target
  that compiles the production files under
  ...
  without even loading the main project's package graph, and without building
  `vreader`, `vreaderUITests`, Readium, or MLX. The focused CI
...
| `BackupCollector` | `LibraryRepository`, `CollectionDao`? | Room library + positions → the #113 manifest (`BlobPath` per book) + positions section + the `BlobUpload` list; loud-fails on a missing local file or a positionless-Readium envelope. With a `CollectionDao` (feature #127 WI-6) also emits a deterministic `collections.json` (`BackupCollectionsEnvelope`, sorted by `nameKey`, keys filtered-to-backed-up + sorted → byte-stable). |
| `RestoreImporter` | `BookImporter`, `LibraryRepository`, `CollectionDao`? | Streams each blob → `BookImporter.importStream(expectedKey=…)` (verify-before-mutate) → restore manifest metadata → restore position (book-first FK). Per-book failure isolation, idempotent. With a `CollectionDao` (#127 WI-6) restores `collections.json` AFTER books (FK holds): merge by `nameKey` (keep existing `createdAt`), union membership for existing books only. |
...
| `Azw3AnnotationMapper` | foliate-js `selection` → (canonical `Locator` + `AnnotationAnchor.Epub` + text); a stored highlight → the CFI for `readerAPI.addAnnotation`, and a tapped CFI → its highlight id. Both resolve **anchor, else `locator.cfi`** — the backup wire carries no anchor, so `restoreAnnotations` inserts `anchor = null`; without the fallback a restored highlight would be unpaintable, and without it in the *tap* path it would be visible but untappable. |
