...
- **Solution (v11, current)**: The `attributedText` setter still crashes at frame #45 deep in UIKit accessibility traversal when the text view has an active selection (from edit menu highlight action). Fix: (1) Clear `selectedTextRange` before replacement to remove stale selection state, (2) Use `textStorage.setAttributedString()` instead of `attributedText =` — bypasses the setter's heavy accessibility processing, (3) Added `isReplacingText` guard flag on `HighlightableTextView` to suppress delegate callbacks (`textViewDidChangeSelection`, `scrollViewDidScroll`) during replacement. Guards added in both bridge coordinators.
- **Lessons**:
  ...
  5. The ONLY safe iOS pattern: build the complete attributed string externally from stored state (never reading the textView), set once via `attributedText =`. All highlight state lives in the coordinator, not the textView.
  6. `beginEditing()/endEditing()` on textStorage triggers `processEditing()` which has internal queue assertions — avoid when the accessibility callback path is blocked.
  7. When using full `attributedText` replacement for highlights, `updateUIView` must compare against the SOURCE text (tracked in coordinator), not the textView's current content. Otherwise the highlight modification triggers re-application → infinite loop.
  8. UITextView.attributedText setter triggers UIKit accessibility traversal that crashes with active selection. Use textStorage.setAttributedString() for safer content replacement — it bypasses the setter's typing attribute reset and accessibility notifications.
