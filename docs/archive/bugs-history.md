...
- **Solution**: Replaced all `textStorage.addAttribute/removeAttribute` for highlights with `layoutManager.addTemporaryAttribute/removeTemporaryAttribute` in both `TXTTextViewBridge` and `TXTChunkedReaderBridge`. Temporary attributes are visual-only — they don't modify the text storage, don't trigger accessibility callbacks, and are Apple's recommended API for search/selection highlights.
- **Lessons**: Never use `textStorage.addAttribute` for visual-only highlights in UITextView. Use `layoutManager.addTemporaryAttribute` instead. Direct textStorage modification triggers UITextViewAccessibility callbacks that can cause infinite recursion. This applies to ANY attribute change on textStorage, not just background colors. `beginEditing()/endEditing()` does not prevent the accessibility callback loop.
